...
> 日期：2026-07-19  
> 状态：历史提案；2026-07-20 已由“原子渐进 HTML + 可信运行时/引擎”实施决策取代
> 决策前提：允许推翻现有生成架构，以通用性、结果多样性和速度为第一优先级。

> 实施偏差说明：实践表明 AppIR JSON 增加模型输出体积和协议失败面，而最终仍需编译回 V2 markup。当前默认改为模型直接输出四阶段闭合 HTML 子树，服务端以 parse5 原子校验/合并；AppIR 保留为兼容与实验路径。本文其余内容保留作为历史决策背景，不再代表默认生成链。

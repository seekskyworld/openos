...
/** 冷生成主协议：HTML 本体保持原生，只用注释边界提交闭合子树。 */
export function buildProgressiveHtmlPrompt(input: {
  name: string;
  description: string;
  query: string;
  tier: CreativityTier;
  ...
  const protocol = [
    ...
    "<!--openos:stage:actions:app-actions--> 后输出以 id=\"app-actions\" 为唯一根的次要操作/状态 HTML，随后输出 <!--openos:end-->。",
    "后面三个块可以引用 shell 或其他已完成块里的 id；每个块必须标签闭合。不要重复整页、不要 JSON、不要转义 HTML。",
  ].join("\n");
  return { system: `${base.system}\n\n【渐进 HTML 协议】\n${protocol}`, user: base.user };
}
